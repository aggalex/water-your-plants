...
use rocket::{delete, get, post, Route, routes, serde::json::Json, State};
...
async fn login(login_dto: Json<UserLoginDto>, db: &State<Pool>, cookie_jar: &CookieJar<'_>) -> Result<(), ErrorResponse> {
    let mut manager = db.get().await?;
    let mut context = TransactionContext::new(Transaction::new(&mut manager).await?);

    let auth_key = cookie_jar.get_private("auth").map(|cookie| cookie.value().to_string());
...
#[post("/logout")]
async fn logout(user: LoggedInUserDTO, db: &State<Pool>, cookie_jar: &CookieJar<'_>) -> Result<(), ErrorResponse> {
    let mut manager = db.get().await?;
    let mut context = TransactionContext::new(Transaction::new(&mut manager).await?);
...
#[post("/", data = "<login_dto>")]
async fn register(login_dto: Json<UserLoginDto>, db: &State<Pool>) -> Result<Json<i32>, ErrorResponse> {
    let mut manager = db.get().await?;
    let mut context = TransactionContext::new(Transaction::new(&mut manager).await?);
...
#[get("/")]
async fn display_info(user: LoggedInUserDTO, db: &State<Pool>) -> Result<Json<UserInfoDto>, ErrorResponse> {
    let mut manager = db.get().await?;
    let mut context = TransactionContext::new(Transaction::new(&mut manager).await?);
...
#[delete("/")]
async fn delete_user(user: LoggedInUserDTO, db: &State<Pool>, cookie_jar: &CookieJar<'_>) -> Result<(), ErrorResponse> {
    let mut manager = db.get().await?;
    let mut context = TransactionContext::new(Transaction::new(&mut manager).await?);
