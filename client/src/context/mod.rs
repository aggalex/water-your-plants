...
pub async fn fetch_uuid() -> Result<(), Box<dyn Error>> {
    let response = connection::get().request(Method::POST, "/node/register").await?;
    set_uuid(serde_json::from_str(&String::from_utf8(response.bytes().await?.to_vec())?)?).await;
    Ok(())
}
...
pub(self) fn var(var_name: &str) -> String {
    std::env::var(var_name)
        .expect(&format!("{var_name} is missing"))
}