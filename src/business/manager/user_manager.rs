...
use crate::persistence::entity::login::{LoginDao, LoginDTO};
...
impl<'a> Injects<'a, UserManager<'a>> for TransactionContext<'a> {
    fn inject(&'a self) -> UserManager<'a> {
        ...
        UserManager {
            user_dao: UsersDao::from(transaction),
            login_dao: LoginDao::from(transaction)
        }
    }
}

impl UserManager<'_> {
    pub async fn login(&self, login_dto: UserLoginDto, auth_key: Option<String>) -> Result<LoginDTO, ErrorResponse> {
        if !auth_key.is_none() {
            return Err(ErrorResponse::BadRequest("You are already logged in".to_string()))
        };
    ...
    pub async fn register(&self, login_dto: UserLoginDto) -> Result<UserInfoDto, ErrorResponse> {
        ...
        let login_dto = UserLoginDto { password, ..login_dto };
