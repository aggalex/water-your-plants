...
impl PlantDao<'_> {
    ...
    pub async fn search_by_belongs_to_user_id(&self, query: Option<&str>, user_id: i32, pagination: PaginationDto) -> QueryResult<Vec<PlantDto>> {
        ...
        self.0.query(&format!(r#"SELECT * FROM plant WHERE belongs_to_user_id = $3 {} LIMIT $1 OFFSET $2"#,
            query.as_ref().map(|_| conditional_clause).unwrap_or("")
        ), &params[..])
            .await.map_err(Into::into).and_then(Vec::<PlantDto>::try_collect)
    }
    ...
    pub async fn get_belongs_to_user_id(&self, plant_id: i32) -> QueryResult<Option<i32>> {
        ...
        let opt = self.0.query_opt(r#"SELECT belongs_to_user_id from plant where id = $1"#, &[&plant_id])
            .await?;
