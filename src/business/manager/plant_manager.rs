...
impl<'a> Injects<'a, PlantManger<'a>> for TransactionContext<'a> {
    fn inject(&'a self) -> PlantManger<'a> {
        ...
        PlantManger {
            ...
            plant_profile_dao: PlantProfileDao::from(transaction),
            conn: DefaultContext.inject()
        }
    }
}
...
impl PlantManger<'_> {
    pub async fn update_moisture(&self, uuid: &str, measurement_dto: &MeasurementDTO) -> Result<(), ErrorResponse> {
        match measurement_dto {
            ...
            MeasurementDTO::Measurement { moisture, .. } => {
                let Some(plant) = self.plant_dao.update_moisture(uuid, moisture.clone()).await? else {
                    return Ok(())
                };
    ...
    pub async fn check_for_watering(&self, plant_dto: PlantDto) -> Result<(), ErrorResponse> {
        ...
        let profile = self.plant_profile_dao.find_by_id(plant_dto.profile_id).await?
            .ok_or(ErrorResponse::InternalServerError(()))?;

        println!("Profile {:?}", profile);

        if plant_dto.soil_moisture < profile.max_moisture && plant_dto.soil_moisture > profile.min_moisture {
            println!("ok");
            return Ok(())
        }
        ...
        }.publish(self.conn.clone()).await.map_err(Into::into)
    }

    pub async fn create_plant(&self, new_plant_dto: NewPlantDto, logged_in_user_dto: LoggedInUserDTO) -> Result<PlantDto, ErrorResponse> {
        let plant = self.plant_dao.create(new_plant_dto, logged_in_user_dto.id).await?;

        Ok(plant)
    }

    pub async fn get_plants_of_user(&self, query: Option<String>, pagination: PaginationDto, logged_in_user_dto: LoggedInUserDTO) -> Result<Vec<PlantDto>, ErrorResponse> {
        let plant = self.plant_dao.search_by_belongs_to_user_id(query.as_ref().map(|s| &**s), logged_in_user_dto.id, pagination).await?;

        Ok(plant)
    }

    pub async fn assert_user_owns_plant(&self, id: i32, logged_in_user_dto: &LoggedInUserDTO) -> Result<(), ErrorResponse> {
    ...
    pub async fn delete_plant(&self, id: i32, logged_in_user_dto: LoggedInUserDTO) -> Result<(), ErrorResponse> {
    ...
    pub async fn update_plant(&self, id: i32, new_plant_dto: NewPlantDto, logged_in_user_dto: LoggedInUserDTO) -> Result<(), ErrorResponse> {
        self.assert_user_owns_plant(id, &logged_in_user_dto).await?;
