...
impl<'r> FromRequest<'r> for LoggedInUserDTO {
    ...
    async fn from_request(request: &'r Request<'_>) -> Outcome<Self, Self::Error> {
        let res: Result<LoggedInUserDTO, Status> = try {
            ...
            let mut context = TransactionContext::new(Transaction::new(&mut manager).await.map_err(|_| Status::InternalServerError)?);
            let tx: &Transaction = context.inject();

            LoginDao::from(tx).get_user_id_of_key(cookie.value()).await
                .map_err(|_| Status::InternalServerError)?
                .map(|id| LoggedInUserDTO { id, key: cookie.value().to_string() })
                .ok_or(Status::Unauthorized)?
        };
