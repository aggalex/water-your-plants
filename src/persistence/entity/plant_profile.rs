...
impl PlantProfileDao<'_> {
    pub async fn search(&self, query: Option<&str>, pagination: &PaginationDto) -> QueryResult<Vec<PlantProfileDto>> {
        ...
        self.0.query(&format!(r#"SELECT * FROM plant_profile {} LIMIT $1 OFFSET $2"#,
            query.as_ref().map(|_| conditional_clause).unwrap_or("")
        ), &params[..])
            .await.map_err(Into::into).and_then(Vec::<PlantProfileDto>::try_collect)
